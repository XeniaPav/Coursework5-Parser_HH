...
def save_data_to_database(data: list[dict[str, Any]], data_company: list[dict[str, Any]], database, params: dict):
    ...
    with conn.cursor() as cur:
        ...
        for company in data:
            ...
            """,
            (company['company_id'], company['company_name'], company['job_title'],
             company['link_to_vacancy'], company['salary_from'], company['salary_to'],
             company['currency'], company['description'], company['requirement'])
             )
